	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/timetable/update_timetable_input.jsp");
					failureView.forward(req, res);
					return;
				}

				TimetableService timetableSvc = new TimetableService();
				timetableVO = timetableSvc.updateTimetable(timetableNo, courseNo, classroomNo, timetablePeriod,
						timetableDate, teachingLog);
		...
		if ("insert".equals(action)) { // 來自addTimetable.jsp的請求
			...
			try {
				...
				TimetableService timetableSvc = new TimetableService();
				timetableVO = timetableSvc.addTimetable(courseNo, classroomNo, timetablePeriod, timetableDate,
						teachingLog);
		...
		if ("delete_teachingLog".equals(action)) {
			...
			try {
				...
				errorMsgs.add("刪除資料失敗:" + e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/teachingnote/TeachingNote.jsp");
				failureView.forward(req, res);
			}
		}
	}
