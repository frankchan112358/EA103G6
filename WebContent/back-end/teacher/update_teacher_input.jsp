...
<title>使用者資料修改 - update_teacher_input.jsp</title>
<style>
a {
...
<body>
	...
	<form method="post" action="<%=request.getContextPath()%>/user.do" enctype="multipart/form-data">
		<table>
			...
			<tr>
				<td>專長：</td>
				<td><input type="text" name="skill" size="40"value="${teacherVO.skill}"></td>
			</tr>
			
			<tr>
				<td>描述：</td>
				<td><textarea name="description" rows="10" cols="50">${teacherVO.description}</textarea></td>
			</tr>
			
			<tr>
				<td>講師狀態：</td>
				<td><select name="teacherStatus">
						<option value="0" <%if (teacherVO.getTeacherStatus() == 0)out.print(" selected");%>>離職
						<option value="1" <%if (teacherVO.getTeacherStatus() == 1)out.print(" selected");%>>在職
						<option value="2" <%if (teacherVO.getTeacherStatus() == 2)out.print(" selected");%>>停職
					</select>
				</td>
			</tr>
